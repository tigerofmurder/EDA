...
class Node{
    public:
        ...
        bool status;
    public:
        Node(int r=0,int g=0, int b=0,int cont=1){
            RED = r;
            GREEN = g;
            BLUE = b;
            contador = cont;
...
class Octree{
    ...
    public:
        ...
        void Quantizer_Color(int numero, Node *&tmp){
	        ...
	        while (q.empty() == false){
		        ...
		        while (nodeCount > 0){
			        ...
			        if (node->pSon[6]!=NULL)q.push(node->pSon[6]);
			        if (node->pSon[7]!=NULL)q.push(node->pSon[7]);
			        //if (node->pSon[8]!=NULL)q.push(node->pSon[8]);
			        nodeCount--;
		        }
		        nvl+=1;
	        }

	        for(int i=nivel_nodes.size()-1;i!=0;i--){
	            for(auto it:nivel_nodes[i]){
	                int hojas = nivel_nodes[0].size()+nivel_nodes[1].size()+nivel_nodes[2].size()+
	                nivel_nodes[3].size()+nivel_nodes[4].size()+nivel_nodes[5].size()+
	                nivel_nodes[6].size()+nivel_nodes[7].size()+nivel_nodes[8].size();
	                if(hojas<=numero){break;}
                    else{
	                    promedio_color(it);
	                    nivel_nodes[i].erase(nivel_nodes[i].begin());
	                }

	            }
	        }
	        for(auto it:nivel_nodes){
	            for (auto et:it.second){
	                pallete.push_back(et);
	            }
	        }
        }
        ...
        //IDE visual;
        vector<int> Reduccion_Image(vector<int> positions, Node *temp){
            vector<int> data;
            if(positions.empty()) return data;
            int elemt= positions.front();
            if(sonEmpty(temp->pSon[elemt])){
                data.push_back(temp->pSon[elemt]->RED);
                data.push_back(temp->pSon[elemt]->GREEN);
                data.push_back(temp->pSon[elemt]->BLUE);

                return data;
            }
            positions.erase(positions.begin());
            return Reduccion_Image(positions,temp->pSon[elemt]);
        }
...
int main()
{
    ...
    //file.display();
    //Raiz.print(Raiz.root);
    cout<<endl<<endl;
    int entrada=256;
    //cin>>entrada;

    Raiz.Quantizer_Color(entrada,Raiz.root);
    cout<<"ELEEMNT: "<<Raiz.Num_color<<endl;
