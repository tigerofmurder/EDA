...
function range_query_circle(node, query, radio, queue, depth = 0){
	if (root==null)
	...
	k_closest_point(next_branch,query,radio,queue,depth+1);
    if (distance <=radio ){

    	insert([root.point[0],root.point[1],distance]);
	}
   	if ( distance > Math.abs(query[axis] - root.point[axis])){
     	 k_closest_point(opposite_branch,query,radio,queue,depth+1);
    }
...
function setup (){
	...
	range_query_circle(root,point,10,lista);
