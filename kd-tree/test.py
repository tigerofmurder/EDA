...
def construccion_las(node,x,w,y,h,nivel=0):
    if(node==None):
        return None
    else:
        if(nivel==0):
            lista.append([(node.point[0],(y+h)/2),(0,abs(h-y)),nivel])
            construccion_las(node.left,x,node.point[0],y,h,1)
            construccion_las(node.right,node.point[0],w,y,h,1)
        if(nivel==1):
            lista.append([((x+w)/2,node.point[1]),(abs(w-x),0),nivel])
            construccion_las(node.left,x,w,y,node.point[1],0)
            construccion_las(node.right,x,w,node.point[1],h,0)
...
def main():
    ...
    if(dimensions == 2):
        ...
        construccion_las(tree,0,400,0,400)
        for i in lista:
            cubo(1+i[0][0],1+i[0][1],0,1+i[1][0],1+i[1][1],0,i[2])
    else:
